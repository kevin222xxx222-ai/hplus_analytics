...
import type { ComparisonDto } from "@/lib/analytics/integration/dto";
...
describe("Trend ViewModel", () => {
  it("maps all comparison axes without recalculating values", () => {
    const comparisons = ["previousDay", "previousWeek", "previousWeekday", "previousMonth", "previousMonthToDate"].map((baselineKind, index) => ({ baselineKind, current: 100, baseline: 90, difference: 10, differenceRate: 0.1, availability: "VALUE", currentAvailability: "VALUE", baselineAvailability: "VALUE", period: { from: "2026-05-01", to: "2026-05-30" }, direction: index === 0 ? "increase" : "flat", confidence: "Medium" })) as unknown as ComparisonDto[];
