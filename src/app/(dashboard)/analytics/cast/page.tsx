...
export default async function CastPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; store?: string; castId?: string }> }) {
  ...
  return <AnalyticsPageLayout layoutOnly><CastAnalyticsPage initialFrom={range.fromText} initialTo={range.toText} initialStore={query.store || "ALL"} /></AnalyticsPageLayout>;
}
