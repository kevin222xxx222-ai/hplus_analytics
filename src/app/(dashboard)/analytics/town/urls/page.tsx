...
export default async function TownUrlAnalyticsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string; scope?: string; pageType?: string; castId?: string }> }) {
  ...
  const records = await prisma.townUrlDaily.findMany({ where: { date: { gte: range.from, lte: range.to }, ...(store ? { storeId: store.id } : {}), ...(pageType ? { pageType } : {}), ...(query.castId ? { castId: query.castId } : {}) }, include: { store: true, cast: true }, orderBy: [{ pv: "desc" }, { url: "asc" }] });
  const total = aggregateTown(records);
  ...
  return <><PageHeader eyebrow="TOWN URL" title="タウンURL分析" description="クエリパラメータを除いた正規化URL単位のアクセス実績です。" /><DateRangeForm from={range.fromText} to={range.toText} extra={extra} />
    <section className="mb-6 grid gap-4 sm:grid-cols-4">{[["PV", n(total.pv)], ["UU", n(total.uu)], ["TEL", n(total.telTapUu)], ["TEL率", total.conversionRate === null ? "—" : `${n(total.conversionRate * 100, 2)}%`]].map(([label, value]) => <div className="panel p-5" key={label}><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-2xl font-semibold">{value}</p></div>)}</section>
    <section className="panel overflow-hidden"><div className="table-wrap"><table><thead><tr><th>URL</th><th>ページ種別</th><th>PV</th><th>UU</th><th>TEL</th><th>TEL率</th><th>キャスト</th><th>店舗</th></tr></thead><tbody>{records.map((row) => <tr key={row.id}><td className="max-w-[520px] break-all font-medium text-slate-900">{row.normalizedUrl}</td><td>{row.pageType}</td><td>{n(row.pv)}</td><td>{n(row.uu)}</td><td>{n(row.telTapUu)}</td><td>{row.conversionRate === null ? "—" : `${n(Number(row.conversionRate) * 100, 2)}%`}</td><td>{row.cast?.displayName || row.sourceCastName || "—"}</td><td>{row.store.shortName}</td></tr>)}</tbody></table>{records.length === 0 && <p className="empty-state">条件に一致するURL実績はありません。</p>}</div></section>
  </>;
}
