...
export default async function CurrentMembershipInitializationPage() {
  ...
  return <>
    ...
    {create.length > 0 && <section className="panel mt-5 p-5"><p className="text-sm">Adminが明示Confirmした場合のみ、サーバー側でPreviewを再取得・再監査したCREATE_ACTIVE候補をACTIVE・joinedAt=NULLで登録します。退店日やFact日付は生成しません。</p>{user.role === "ADMIN" ? <form action={initializeCurrentMembershipsAction} className="mt-3"><input type="hidden" name="confirm" value="CONFIRM" /><button className="primary-button" disabled={validationError}>現在在籍Membershipを初期登録（Confirm）</button></form> : <p className="mt-3 text-sm text-slate-500">ViewerはPreviewのみ閲覧できます。</p>}</section>}
  </>;
}
