...
export async function GET(request: Request) {
  try {
    ...
    try { return Response.json(await getPerformance(parseAnalyticsParams(url.searchParams))); } catch (error) { throw new ApiError(error instanceof Error ? error.message : "不正な分析条件です。", 400); }
