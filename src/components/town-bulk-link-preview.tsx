...
const CATEGORY_LABELS: Record<TownBulkLinkCategory, string> = { A: "A：自動紐付け可能", B: "B：管理者一括承認候補", C: "C：個別確認・保留", D: "D：再解析待ち" };
const REASON_LABELS: Record<string, string> = {
  ID_FORMAT: "ID:数字形式", ID_NO_SOURCE_URL: "ID・元URLなし（再解析待ち）", CORRECTION_CANDIDATE: "修正版候補", MULTIPLE_CANDIDATES: "候補複数",
...
function DeferredSourceSummary({ candidates }: { candidates: TownBulkLinkCandidate[] }) {
  const byStore = new Map<string, { people: number; rows: number }>();
  for (const candidate of candidates) {
    const current = byStore.get(candidate.storeName) || { people: 0, rows: 0 };
    current.people += 1; current.rows += candidate.rowCount; byStore.set(candidate.storeName, current);
  }
  return <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900"><p className="font-semibold">D：再解析待ち</p><p className="mt-1 text-xs">ID形式で、preview.jsonから元URLを取得できない候補です。ImportErrorはOPENのまま保持し、実績・Alias・Castは変更しません。URL取得後の再解析でCへ復帰します。</p><div className="mt-3 flex flex-wrap gap-3">{[...byStore.entries()].map(([store, value]) => <span key={store}>{store}：{value.people}人 / {value.rows.toLocaleString("ja-JP")}行</span>)}</div></div>;
}
...
function CWorkspace({ preview, request, refreshPreview, onBatchChanged }: { preview: TownBulkLinkPreview; request: (body: Record<string, unknown>) => Promise<unknown>; refreshPreview: (message?: string) => Promise<void>; onBatchChanged?: () => Promise<void> }) {
  ...
  const [drafts, setDrafts] = useState<Record<string, NewCastDraft>>({}); const [skipReasons, setSkipReasons] = useState<Record<string, string>>({}); const skipReasonsRef = useRef<Record<string, string>>({}); const [executionResults, setExecutionResults] = useState<Record<string, string>>({});
  ...
  async function executeCandidate(candidate: TownBulkLinkCandidate, operation: "EXISTING" | "NEW" | "SKIP", extras: Record<string, unknown>) {
    ...
    try {
      const result = await request({ action: "EXECUTE_CANDIDATE", candidateKey: candidate.key, fingerprint: preview.fingerprint, operation, ...extras }) as { resolvedRows: number; skippedRows?: number; affectedBatchCount: number; createdCastId: string | null; aliasId?: string | null; insertedFacts: number; kindCounts?: { cast: number; url: number; landing: number }; remainingIdCandidates?: number };
      const summary = operation === "SKIP"
        ? `ID ${candidate.townName}を除外：${result.skippedRows || result.resolvedRows}行 / ${result.affectedBatchCount}バッチ・CAST ${result.kindCounts?.cast || 0} / URL ${result.kindCounts?.url || 0} / LP ${result.kindCounts?.landing || 0}・残りID候補 ${result.remainingIdCandidates ?? "—"}`
        : `解決 ${result.resolvedRows}行 / ${result.affectedBatchCount}バッチ・Alias ${result.aliasId}・追加実績 ${result.insertedFacts}件${result.createdCastId ? `・作成Cast ${result.createdCastId}` : ""}`;
      setExecutionResults((current) => ({ ...current, [candidate.key]: summary }));
      await refreshPreview(summary);
      // The bulk file table is owned by the parent component and keeps its
      // own scan snapshot. Refresh it after a successful candidate action so
      // ImportBatch status/counts do not remain visually stale.
  ...
  return <div className="min-w-0 space-y-4">
    <DeferredSourceSummary candidates={preview.candidates.filter((candidate) => candidate.category === "D")} />
      ...
      return <>
        ...
        {isOpen && <tr key={`${candidate.key}:detail`}><td colSpan={12} className="!bg-white !p-4 whitespace-normal"><div className="min-w-0 space-y-4">
          ...
          {actions.includes("CORRECTION_DIFF") ? <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs"><p className="font-semibold">修正版候補は通常のAlias処理対象外です。</p><p className="mt-1">差分プレビュー・既存完了版維持・修正版採用はPhase 3で安全な置換設計とともに実装します。現時点では保留のみ記録できます。</p><div className="mt-3 flex flex-wrap gap-2"><button type="button" className="secondary-button" onClick={() => void inspect(candidate, "CORRECTION_REVIEW")}>差分プレビュー（Phase 3）</button><button type="button" className="secondary-button" onClick={() => void inspect(candidate, "PENDING")}>既存完了版を維持／保留</button><button type="button" className="secondary-button" disabled>修正版を採用（実行不可）</button></div></div> : <>
            ...
            /></div>}
            <div className="flex flex-wrap items-end gap-2">{actions.includes("SKIP") && <><label className="min-w-64 text-xs">今回除外理由（必須）<input className="town-candidate-input mt-1" value={skipReasons[candidate.key] || ""} onChange={(event) => { skipReasonsRef.current[candidate.key] = event.target.value; setSkipReasons((current) => ({ ...current, [candidate.key]: event.target.value })); setImpacts((current) => { const next = { ...current }; delete next[candidate.key]; return next; }); }} /></label><button type="button" className="secondary-button" disabled={!skipReasons[candidate.key]?.trim() || pendingKey === candidate.key} onClick={() => void inspect(candidate, "SKIP", { skipReason: skipReasonsRef.current[candidate.key] || skipReasons[candidate.key] || "" })}>今回除外の影響を確認</button>{candidate.reasonCodes[0] === "ID_FORMAT" && <><button type="button" className="primary-button" disabled={pendingKey === candidate.key || impact?.operation !== "SKIP" || !impact.executable || !skipReasons[candidate.key]?.trim()} onClick={() => { if (window.confirm(`${candidate.townName} の${candidate.rowCount}行・${candidate.batchCount}バッチを今回除外します。対象行のPV・UU・TELは実績へ入りません。実行しますか？`)) void executeCandidate(candidate, "SKIP", { skipReason: skipReasonsRef.current[candidate.key] || skipReasons[candidate.key] || "" }); }}>今回除外を実行</button><button type="button" className="secondary-button" disabled={pendingKey === candidate.key} onClick={() => { skipReasonsRef.current[candidate.key] = ""; setSkipReasons((current) => ({ ...current, [candidate.key]: "" })); setImpacts((current) => { const next = { ...current }; delete next[candidate.key]; return next; }); }}>入力内容をリセット</button><p className="basis-full text-xs text-amber-800">{!skipReasons[candidate.key]?.trim() ? "除外理由を入力してください。" : impact?.operation !== "SKIP" ? "影響範囲を確認してください。" : impact && !impact.executable ? impact.stopReasons.join(" ") : ""}</p></>}</>}<button type="button" className="secondary-button" onClick={() => void inspect(candidate, "PENDING")}>保留（画面上のみ）</button></div>
          </>}
...
export function TownBulkLinkPreviewPanel({ onBatchChanged }: { onBatchChanged?: () => Promise<void> } = {}) {
