...
export function HeavenUploadForm({ store }: { store: { id: string; name: string } }) {
  ...
  return <form onSubmit={submit} className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 md:items-end">
    ...
    <div><span className="form-label">店舗</span><div className="form-input mt-2 bg-slate-50" aria-label="店舗">{store.name}</div></div>
    ...
    <button disabled={pending} className="primary-button md:col-span-2 xl:col-span-1">{pending ? <LoaderCircle className="size-4 animate-spin" /> : <UploadCloud className="size-4" />}{pending ? "検証中…" : "検証・プレビュー"}</button>
