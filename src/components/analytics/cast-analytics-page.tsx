...
export function CastAnalyticsPage({ initialFrom, initialTo, initialStore, initialCastId }: { initialFrom: string; initialTo: string; initialStore: string; initialCastId: string }) {
  ...
  return <AnalyticsPageLayout layoutOnly loading={detail.loading}>
    <div className="mx-auto max-w-[1500px] space-y-6">
      <AnalyticsHeader title="Cast Analytics" eyebrow="キャスト分析" description="原因・根拠・アクションの順で、1名のCTI・Town・Heaven実績を確認します。" period={`${filters.from}〜${filters.to}`} loading={detail.loading} onRefresh={detail.retry} />
      ...
      {!castId ? <AnalyticsUnavailableMessage message="キャストを選択してください。候補一覧を読み込んで選択できます。" /> : detail.error ? <AnalyticsErrorMessage message={detail.error} onRetry={detail.retry} /> : detail.loading && !detail.data ? <AnalyticsLoadingState /> : !detail.data || !summary ? <AnalyticsUnavailableMessage message="指定キャストの分析データは利用できません。" /> : <>
        <AnalyticsSection title="Cause"><article className="panel p-5"><h3 className="text-lg font-semibold">{detail.data.cast.cast?.displayName ?? "—"}</h3><p className="mt-2 text-sm text-slate-600">改善ポイント: {summary.growth?.classification ?? "—"}</p><p className="text-sm text-slate-600">信頼度: {summary.sample.confidence} / データ状態: {summary.growth?.availability ?? "MISSING"}</p></article></AnalyticsSection>
        <AnalyticsSection title="Evidence"><AnalyticsSection title="Cast Summary" description="集計値とAvailability／Confidence／Sampleを確認します。"><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">{castMetricKeys.slice(0, 7).map((key) => <article className="panel p-4" key={key}><p className="text-xs text-slate-500">{castMetricLabels[key]}</p><p className="mt-2 text-xl font-semibold">{formatMetric(value(key), castMetricFormats[key])}</p><p className="mt-1 text-xs text-slate-500">{availability(key)} / {summary.sample.confidence}</p></article>)}</div></AnalyticsSection>
          <AnalyticsSection title="Performance"><AnalyticsMetricGroup title="Volume" metrics={castMetricKeys.filter((key) => ["sales", "castReward", "attendancePeople", "reservations", "services", "regularNominations", "diaryPosts"].includes(key)).map((key) => ({ key, label: castMetricLabels[key], value: value(key), format: castMetricFormats[key] === "pv" || castMetricFormats[key] === "uu" ? "integer" : castMetricFormats[key], hint: availability(key) }))} /><AnalyticsMetricGroup title="Efficiency" metrics={castMetricKeys.filter((key) => ["salesPerHour", "rewardPerHour"].includes(key)).map((key) => ({ key, label: castMetricLabels[key], value: value(key), format: "currency", hint: availability(key) }))} /></AnalyticsSection>
        ...
        <AnalyticsSection title="Action"><article className="panel p-5"><h2 className="text-lg font-semibold">改善ポイント</h2><p className="mt-2">{summary.growth?.classification ?? "—"}</p><p className="mt-3 text-sm text-slate-600">{summary.growth?.evidence?.join(" / ") || summary.growth?.reason || "根拠データがありません。"}</p><h2 className="mt-5 text-lg font-semibold">推奨アクション</h2><p className="mt-2 text-sm">{summary.nextBestAction?.action ?? "提案なし"}</p><p className="mt-2 text-xs text-slate-500">原因: {summary.nextBestAction?.cause ?? "—"} / 信頼度: {summary.nextBestAction?.confidence ?? "Insufficient"}</p></article></AnalyticsSection>
