...
export function CastStartDateMaintenance({ candidates }: { candidates: Candidate[] }) {
  ...
  return <div className="space-y-6">
    ...
    {preview && <section ref={previewSectionRef} className="panel scroll-mt-6 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">変更プレビュー</h2><p className="mt-1 text-sm text-slate-500">Cast {preview.castChanges.length}名 / Alias {preview.aliasChanges.length}件 / 対象日 {preview.targetDate}</p></div>{preview.conflicts.length ? <span className="status-badge bg-red-50 text-red-700"><AlertTriangle className="size-4" />停止対象 {preview.conflicts.length}件</span> : preview.safeAliasMerges.length ? <span className="status-badge bg-sky-50 text-sky-700"><CheckCircle2 className="size-4" />統合予定 {preview.safeAliasMerges.reduce((count, merge) => count + merge.mergedAliasIds.length, 0)}件</span> : <span className="status-badge bg-emerald-50 text-emerald-700"><CheckCircle2 className="size-4" />衝突なし</span>}</div>
      ...
      {preview.safeAliasMerges.length > 0 && <div className="mt-4 rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-950"><p className="font-semibold">安全に統合できる重複Alias（{preview.safeAliasMerges.length}組）</p><div className="mt-3 space-y-3">{preview.safeAliasMerges.map((merge) => <div key={merge.key} className="rounded-lg border border-sky-200 bg-white/70 p-3"><p className="font-medium">{merge.castName} / {merge.normalizedAlias}</p><p className="mt-1 text-xs text-slate-600">{merge.mediaType} / {merge.storeId || "店舗共通"} / 変更後 validFrom：{preview.targetDate}</p><p className="mt-2 text-xs">代表Alias：{merge.representativeAliasId}（{merge.representativeValidFrom} → {preview.targetDate}）</p><p className="text-xs">統合対象：{merge.mergedAliasIds.map((id, index) => `${id}（${merge.mergedValidFrom[index]} → 統合）`).join("、")}</p></div>)}</div></div>}
