...
export function validateTownStoreExecuteInput(input: Pick<TownStoreExecuteInput, "driveFileId" | "targetDate">): void {
...
export function assertTownStoreProductionExecution(environment: string | undefined, confirmProduction: boolean): void {
  if (environment === "production" && !confirmProduction) throw new Error("Production execution requires --confirm-production.");
}

export function assertTownStoreMapping(mapping: {
  isActive: boolean;
  isFuture: boolean;
  importDataType: ImportDataType;
  storeId: string | null;
  importSource: { mediaType: MediaType; dataType: ImportDataType; storeId: string | null; store?: { code: StoreCode } | null };
}): void {
  if (!mapping.isActive || mapping.isFuture) throw new Error("Drive mapping is inactive or future.");
  if (mapping.importDataType !== ImportDataType.TOWN_STORE || mapping.importSource.dataType !== ImportDataType.TOWN_STORE || mapping.importSource.mediaType !== MediaType.TOWN) throw new Error("Drive mapping is not a valid TOWN_STORE mapping.");
  if (!mapping.storeId || mapping.storeId !== mapping.importSource.storeId) throw new Error("TOWN_STORE mapping requires a matching storeId.");
  if (!mapping.importSource.store || mapping.importSource.store.code === StoreCode.NODA || mapping.importSource.store.code === StoreCode.KUKI) throw new Error("TOWN_STORE mapping is restricted to a supported store.");
}
...
export function sameTownStoreIdentity(metadata: unknown, state: { driveFileId: string; driveModifiedTime: Date; sha256: string | null }, stateBatch: { metadata: unknown } | null): boolean {
  const data = metadataObject(stateBatch?.metadata);
  return data.origin === "GOOGLE_DRIVE"
    && data.importDataType === ImportDataType.TOWN_STORE
  ...
  const locked = await withAdvisoryLock<TownStoreExecuteResult>(driveFileLockName(input.driveFileId), async () => {
    ...
    try {
      ...
      const duplicate = await prisma.importBatch.findFirst({
        where: { fileHash: downloaded.sha256, dataType: ImportDataType.TOWN_STORE, status: { in: ["COMPLETED", "COMPLETED_WITH_WARNINGS"] } },
      ...
      const preview = await createTownPreview({
        file: new File([buffer], downloaded.fileName, { type: downloaded.mimeType || "text/csv" }),
        importSourceId: mapping.importSourceId, storeId: mapping.storeId!, dataType: ImportDataType.TOWN_STORE,
        targetFrom: input.targetDate, targetTo: input.targetDate, uploadedByUserId: actor.id,
        metadata: { origin: "GOOGLE_DRIVE", importDataType: ImportDataType.TOWN_STORE, driveFileId: state.driveFileId, driveModifiedTime: state.driveModifiedTime.toISOString(), driveSha256: downloaded.sha256, driveFileStateId: state.id, executionMode: "EXECUTE", reviewRequired: true },
      });
