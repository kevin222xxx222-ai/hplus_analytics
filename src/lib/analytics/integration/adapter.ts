...
export function adaptSnapshot(snapshot: AnalyticsQuerySnapshot) {
  ...
  const heavenRows: AnalyticsRow[] = snapshot.heaven.map((row) => {
    ...
    return { date: row.businessDate, storeId: row.storeId, castId: row.castId, media: "HEAVEN", naturalKey: `heaven:${row.importBatchId}:${formatDateOnly(row.businessDate)}:${row.storeId}:${row.resolutionKey}:${row.metricKey}`, metrics: { heavenAccess: row.metricKey === "page_access" && isValue ? decimal(row.rawValue) : null, heavenDiaryPosts: row.metricKey === "diary_posts" && isValue ? decimal(row.rawValue) : null, heavenMiteneSent: row.metricKey === "mitene_sent" && isValue ? decimal(row.rawValue) : null } };
  });
