...
/** Fields shared by the normal Heaven Confirm success paths. Failure and duplicate-cancel paths are separate. */
export function heavenConfirmCompletionFields(pending: number, completedAt = new Date()) {
  return { status: pending ? ImportBatchStatus.COMPLETED_WITH_WARNINGS : ImportBatchStatus.COMPLETED, completedAt } as const;
}

export async function createHeavenPreview(input: { file: File; storeId: string; metricHint?: HeavenMetricType; uploadedByUserId: string; metadata?: Record<string, unknown> }) {
  ...
  try {
    ...
    await prisma.importBatch.update({ where: { id: batchId }, data: { status, pendingCount: unresolved.length, warningCount: preview.warningCount, errorCount: 0, detectedColumns: { headers: parsed.headers, metricType: preview.metricType, valueKind: preview.valueKind }, metadata: { ...driveMetadata, metricHint: input.metricHint || null, metricType: preview.metricType, valueKind: preview.valueKind, sourcePeriodFrom: parsed.sourcePeriodFrom, sourcePeriodTo: parsed.sourcePeriodTo, duplicateCompletedBatchId: null } } });
...
export async function confirmHeavenImport(batchId: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const pending = preview.unmatchedCount + preview.ambiguousCount; const current = await tx.importBatch.findUnique({ where: { id: batchId }, select: { metadata: true } }); const base = current?.metadata && typeof current.metadata === "object" && !Array.isArray(current.metadata) ? current.metadata as Record<string, unknown> : {}; await tx.importBatch.update({ where: { id: batchId }, data: { ...heavenConfirmCompletionFields(pending), insertedCount: inserted, updatedCount: updated, pendingCount: pending, warningCount: preview.warningCount, metadata: JSON.parse(JSON.stringify({ ...base, importEvents: [...((base.importEvents as unknown[]) || []), { type: "HEAVEN_INITIAL_CONFIRM", at: new Date().toISOString(), inserted, updated, pending } ] })) } }); return { batchId, inserted, updated, pending };
  });
}
