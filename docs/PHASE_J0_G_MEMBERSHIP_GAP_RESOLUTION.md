...
The audit artifact now contains a complete Cast×Store trace: Membership state, Legacy status/date, primary store, Town/CTI dataset and Batch identifiers, Alias/Listing/Heaven evidence, retired-marker/conflict flags, exclusive decision, and explicit review reasons. It also separates the membership-free population from the existing-Membership population and validates the reconciliation equations before any Apply can be considered. Decision counts are exclusive; a row may carry multiple evidence/review reasons, so reason totals are intentionally non-exclusive and include both Cast and Cast×Store counts.
