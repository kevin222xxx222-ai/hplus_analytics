...
async function main() {
  ...
  const merged = await prisma.cast.findMany({ where: { mergedIntoCastId: { not: null } }, select: { id: true, displayName: true, mergedIntoCastId: true, mergedAt: true, memberships: { where: { status: { in: ["ACTIVE", "ON_LEAVE"] } }, select: { id: true, storeId: true, status: true, joinedAt: true, leftAt: true, source: true, sourceConfidence: true } }, mediaListings: { where: { isListed: true }, select: { id: true, storeId: true, mediaType: true, listedFrom: true, listedTo: true } } });
